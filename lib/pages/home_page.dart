...
class _HomePageState extends State<HomePage> {
  ...
  Future<void> _printCrimeReports() async {
    ...
    for (final report in reports) {
      print('ID: ${report.id}, Photos: ${report.photoPaths}'); // ✅ UPDATE: photoPaths
    }
  }
  ...
  Future<void> _toggle(CrimeReport c) async {
    final upd = CrimeReport(
      ...
      severity: c.severity,
      resolved: !c.resolved,
      photoPaths: c.photoPaths, // ✅ UPDATE: photoPaths
    );
  ...
  Future<void> _delete(CrimeReport c) async {
    ...
    if (ok == true) {
      ...
      for (final photoPath in c.photoPaths) {
        try {
  ...
  Widget _buildHeader(BuildContext context) {
    ...
    if (_qName?.isNotEmpty ?? false) {
      filters.add(_FilterChip(label: 'Name: $_qName', onClear: () => setState(() => _qName = null)));
    }
    if (_fromDate != null) {
      filters.add(_FilterChip(label: 'From: ${_fmtDate(_fromDate)}', onClear: () => setState(() => _fromDate = null)));
    }
    if (_toDate != null) {
      filters.add(_FilterChip(label: 'To: ${_fmtDate(_toDate)}', onClear: () => setState(() => _toDate = null)));
    }
        ...
        children: [
          Flexible(
            ...
            child: Wrap(
              ...
              children: [
                Text('Results: ${items.length}', style: theme.textTheme.titleMedium),
              ],
            ),
          ),
          IconButton(
          ...
          // IconButton(
          //   onPressed: () {
          //     Navigator.push(context, MaterialPageRoute(builder: (_) => const TestDataPage()));
          //   },
...
class _PinnedHeader extends SliverPersistentHeaderDelegate {
  _PinnedHeader({required this.child, required this.min, required this.max});
  final Widget child;
  final double min;
  final double max;
